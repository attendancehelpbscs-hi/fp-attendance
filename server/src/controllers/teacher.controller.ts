...
import { sendTeacherApprovalEmail, sendTeacherRejectionEmail, sendTeacherWelcomeEmail, sendTeacherDeletionEmail } from '../helpers/email.helper';
...
export const deleteTeacherById = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    // Store teacher info before deletion for email notification
    ...
    // Get admin name for email notification
    ...
    // Send deletion notification email and capture result for audit log
    let emailNotificationStatus = 'pending';
    try {
      const emailSent = await sendTeacherDeletionEmail(teacherEmail, teacherName, adminName, deletionTimestamp);
      emailNotificationStatus = emailSent ? 'sent_successfully' : 'failed_to_send';
      if (emailSent) {
        console.log(`✅ Deletion notification email sent to ${teacherEmail}`);
      } else {
        console.log(`⚠️ Failed to send deletion notification email to ${teacherEmail}`);
      }
    } catch (error) {
      emailNotificationStatus = 'error_sending';
      console.error('❌ Error sending deletion notification email:', error);
    }

    // Audit log
    await createAuditLog(user_id, 'TEACHER_DELETED', `Teacher ${teacher.email} deleted by admin after password verification. Email notification: ${emailNotificationStatus}`);

    return createSuccess(res, 200, 'Teacher deleted successfully', { deleted: true, emailNotificationStatus });
  } catch (err) {
    return next(err);
  }
};
