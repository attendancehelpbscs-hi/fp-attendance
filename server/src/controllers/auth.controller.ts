...
export const loginTeacher = async (req: Request, res: Response, next: NextFunction) => {
  // TEST CODE - VERIFY SERVER IS RUNNING UPDATED CODE
  console.log('🔍 TEACHER LOGIN FUNCTION CALLED - DEBUG MODE ACTIVE');
  console.log('🚀 Server running UPDATED loginTeacher function');
  ...
  try {
    console.log('Calling getStaffFromDb...');
    const loggedInTeacher = await getStaffFromDb(email, password);
    console.log('getStaffFromDb result:', loggedInTeacher ? 'Success' : 'No result');
