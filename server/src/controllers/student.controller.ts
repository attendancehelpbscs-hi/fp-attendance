...
export const createStudent = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await markAbsentForStudent(savedStudent.id, today, staff_id);
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // If confidence is above 25%, consider it a duplicate (lowered from 70% to catch actual duplicates)
        if (confidence > 25 && result.student_id) {
          return createSuccess(res, 200, 'Fingerprint already exists', {
            isUnique: false,
            confidence,
            matchedStudent: {
