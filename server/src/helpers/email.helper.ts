...
export const sendTeacherDeletionEmail = async (email: string, teacherName: string, adminName: string, deletionTimestamp: Date) => {
  const formattedTimestamp = deletionTimestamp.toLocaleString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
  ...
  const mailOptions = {
    from: envConfig.EMAIL_USER,
    to: email,
    subject: 'Important: Your Teacher Account Has Been Deleted - Fingerprint-Based Attendance Monitoring System',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background-color: #DC2626; color: white; padding: 20px; border-radius: 5px 5px 0 0; text-align: center;">
          <h1 style="margin: 0;">🚨 Account Deletion Notification</h1>
        </div>
        <div style="border: 1px solid #dee2e6; border-top: none; padding: 20px; border-radius: 0 0 5px 5px;">
          <h2 style="color: #333;">Dear ${teacherName},</h2>
          <p>This email is to notify you that your teacher account for the <strong>Fingerprint-Based Attendance Monitoring System</strong> has been <span style="color: #DC2626; font-weight: bold;">permanently deleted</span> by an administrator.</p>

          <div style="background-color: #fef2f2; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #DC2626;">
            <p style="margin: 0; color: #991b1b;"><strong>⚠️ Important Information:</strong></p>
            <ul style="margin: 10px 0 0 0; color: #991b1b;">
              ...
              <li>All associated data has been removed</li>
              <li>This action cannot be undone</li>
            </ul>
          </div>

          <h3 style="color: #DC2626;">Deletion Details:</h3>
          <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0;">
            <p style="margin: 5px 0;"><strong>Deleted Account:</strong> ${teacherName} (${email})</p>
            <p style="margin: 5px 0;"><strong>Deletion Date & Time:</strong> ${formattedTimestamp}</p>
            <p style="margin: 5px 0;"><strong>Action Performed By:</strong> Administrator</p>
            <p style="margin: 5px 0;"><strong>Reason:</strong> Administrative action</p>
          </div>

          <h3 style="color: #374151;">What This Means:</h3>
          <ul>
            <li><strong>No System Access:</strong> You will no longer be able to log into the attendance system</li>
            <li><strong>No Data Access:</strong> All your teacher data, student records, and attendance history have been removed</li>
            <li><strong>Email Access:</strong> This email address may still receive system notifications from other sources</li>
            <li><strong>Account Recovery:</strong> There is no way to recover a deleted account</li>
          </ul>

          <h3 style="color: #374151;">If You Believe This Was a Mistake:</h3>
          <div style="background-color: #fef3c7; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #F59E0B;">
            <p style="margin: 0; color: #92400e;"><strong>🔍 Concerned about this deletion?</strong></p>
            <p style="margin: 10px 0 0 0; color: #92400e;">If you did not expect your account to be deleted or believe this was done in error, please contact the system administrator immediately with the following information:</p>
            <ul style="margin: 10px 0 0 0; color: #92400e;">
              <li>This email notification</li>
              <li>Your teacher ID or email address</li>
              <li>Any relevant details about your account status</li>
              <li>Confirmation that this deletion was unexpected</li>
            </ul>
          </div>

          <div style="background-color: #f0f9ff; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #0EA5E9;">
            <p style="margin: 0; color: #0c4a6e;"><strong>📧 For Administrative Inquiries:</strong></p>
            <p style="margin: 10px 0 0 0; color: #0c4a6e;">Please contact your system administrator or IT department if you have questions about this deletion or need to discuss account restoration options.</p>
          </div>

          <p>If this deletion was intentional and authorized, no further action is required.</p>
          ...
          <hr>
          <p style="color: #666; font-size: 12px;">Fingerprint-Based Attendance Monitoring System - Administrative Notification</p>
          <p style="color: #666; font-size: 12px;">This is an automated notification sent for security and transparency purposes.</p>
        </div>
      </div>
    `,
  };
  ...
  // Always log the email details to console for debugging/fallback
  console.log('=== TEACHER DELETION EMAIL ===');
  console.log('To:', email);
  console.log('Teacher Name:', teacherName);
  console.log('Admin Name:', adminName);
  console.log('Deletion Timestamp:', formattedTimestamp);
  console.log('Subject:', mailOptions.subject);
  console.log('==============================');
  ...
  if (envConfig.EMAIL_USER && envConfig.EMAIL_PASS) {
    try {
      await transporter.sendMail(mailOptions);
      console.log('✅ Teacher deletion email sent successfully via Gmail');
      return true;
    } catch (error) {
      console.error('❌ Failed to send teacher deletion email via Gmail:', error instanceof Error ? error.message : String(error));
      console.log('📝 Email details logged to console as fallback');
      return false;
    }
  } else {
    console.log('📧 Gmail credentials not configured - deletion email details logged to console only');
    return false;
  }
};
