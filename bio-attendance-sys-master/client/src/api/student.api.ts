...
export const useGetStudents = (staffId: string, page = 1, per_page = 10, options: Omit<UseQueryOptions<GetStudentsResult, BaseError>, 'queryFn'> = {}) =>
  useBaseQuery<GetStudentsResult, BaseError>(`/api/students/staff/${staffId}?page=${page}&per_page=${per_page}`, options);
export const useGetStudentsFingerprints = (staffId: string, options: Omit<UseQueryOptions<GetStudentsFingerprintsResult, BaseError>, 'queryFn'> = {}) =>
  useBaseQuery<GetStudentsFingerprintsResult, BaseError>(`/api/students/fingerprints/${staffId}`, options);
