  ...
  return useQuery(
    ...
    async () => {
      const response = await api.get<ApiResponse<{ students: Student[]; meta: PaginationMeta }>>(
        `/students/staff/${staffId}?page=${page}&per_page=${perPage}`
      );
...
export const useUpdateStudent = () => {
  ...
  return useMutation(
    async (data: { id: string; name?: string; matric_no?: string; grade?: string; fingerprint?: string; courses?: string[]; url: string }) => {
      const response = await api.put<ApiResponse<{ student: Student }>>(`/student${data.url}`, {
        name: data.name,
...
export const useAddStudent = (options?: any) => {
  ...
  return useMutation(
    async (data: AddStudentInput) => {
      const response = await api.post<ApiResponse<{ student: Student }>>('/student', data);
      return response.data;
    },
    {
      onSuccess: () => {
...
export const useImportStudents = (options?: any) => {
  ...
  return useMutation(
    async (formData: FormData) => {
      const response = await api.post<ApiResponse<ImportStudentsResult>>('/import/students', formData, {
        headers: {
...
export const useDeleteStudent = (options?: any) => {
  ...
  return useMutation(
    async (data: { url: string }) => {
      const response = await api.delete<ApiResponse<{ message: string }>>(`/student${data.url}`);
      return response.data;
    },
    {
      onSuccess: () => {
  ...
  return useQuery(
    ...
    async () => {
      const response = await api.get<ApiResponse<{ students: StudentFingerprint[]; meta: PaginationMeta }>>(
        `/students/fingerprints/${staffId}?page=${page}&per_page=${perPage}`
      );
      return response.data;
    },
    {
  ...
  return useMutation(
    async (fingerprint: string) => {
      const response = await api.post<ApiResponse<{
        ...
        message: string;
      }>>('/student/check-fingerprint', { fingerprint });
      return response.data;
    }
  );
};
...
export const useGetStudentFingerprints = (studentId: string, options?: any) => {
  return useQuery(
    ...
    async () => {
      ...
      }>>(`/student/${studentId}/fingerprints`);
      return response.data;
    },
    {
      enabled: !!studentId,
...
export const useAddStudentFingerprint = (options?: any) => {
  ...
  return useMutation(
    async (data: {
      ...
      }>>('/student/fingerprint/enroll', data);
      return response.data;
    },
    {
  ...
  return useMutation(
    async (data: { fingerprint_id: string; student_id: string }) => {
      const response = await api.delete<ApiResponse<{ message: string }>>(
        `/student/fingerprint/${data.fingerprint_id}`
      );
      return response.data;
    },
    {
  ...
  return useMutation(
    async (data: { fingerprint: string; excludeStudentId?: string }) => {
      const response = await api.post<ApiResponse<{
        ...
        message: string;
      }>>('/student/check-fingerprint-multi', data);
      return response.data;
    }
  );
};