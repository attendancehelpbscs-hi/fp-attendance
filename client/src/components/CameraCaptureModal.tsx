  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        // Specific check for insecure context on non-localhost
        if (!isSecure && !isLocalhost) {
          console.warn('⚠️ [DEBUG] Camera requires Secure Context (HTTPS) on non-localhost');
          setHasPermission(false);
          setError(`Camera access requires a secure connection (HTTPS). You are currently using ${window.location.protocol}//${hostname} which is treated as insecure by the browser. Please use localhost or set up HTTPS.`);
          return;
        }
        ...
        if (isDevelopment) {
          console.log('🔧 [DEBUG] Development environment detected, providing mock camera...');
          // In development, we can provide a mock or better error message
          setHasPermission(false);
          setError('Camera API not available. If you are on a local network (192.168.x.x), browser security blocks camera access over HTTP. Please use localhost or HTTPS.');
          return;
        }
        
        throw new Error('Camera API not supported in this browser. Please use a modern browser like Chrome, Firefox, Safari, or Edge with camera permissions enabled.');
      }
      ...
      } else if (err.message?.includes('MediaDevices') || err.message?.includes('mediaDevices')) {
        errorMessage += 'MediaDevices API not available. This may be due to browser security restrictions, testing environment limitations, or outdated browser.';
      } else {
        ...
        >
          ...
          <Box mt={4}>
            ...
            >
              ...
              <Text>navigator.mediaDevices: {!!navigator.mediaDevices ? 'Available' : 'Not Available'}</Text>
              <Text>navigator.mediaDevices.getUserMedia: {!!navigator.mediaDevices?.getUserMedia ? 'Available' : 'Not Available'}</Text>
              <Text>Browser: {navigator.userAgent.includes('Chrome') ? 'Chrome' :
                             navigator.userAgent.includes('Firefox') ? 'Firefox' :
                             navigator.userAgent.includes('Safari') ? 'Safari' :
                             navigator.userAgent.includes('Edge') ? 'Edge' : 'Unknown'}</Text>
